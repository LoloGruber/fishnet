...
public:
    ...
    void run() override{
        ...
        if(inputs.empty())
            throw std::runtime_error("No input file(s) provided");
        ...
        for(const auto & shp : inputs) {
            ...
            auto fileRef = graph.getAdjacencyContainer().getDatabaseConnection().addFileReference(shp.getPath().filename().string()); // load file reference from database
            if(not fileRef){
