...
TEST_F(MemgraphQueryTest, mergeAndMatchEdge){
    ...
    i.merge(DirectedRelation{.from={.name="f"},.label=Label::neighbours,.to={.name="t"}});
    EXPECT_TRUE(i.executeAndDiscard(connection));
    CipherQuery q = CipherQuery().match(DirectedRelation{.from={.name="f",.label=Label::Settlement},.to={.name="t",.label=Label::Settlement}}).ret("f","t");
